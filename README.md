...
단순한 파이썬 스크립트 작성에 그치지 않고, 웹 애플리케이션의 프론트엔드와 백엔드를 연계하고 외부 API 데이터 파이프라인의 실시간 안정성을 확보하는 등 대용량 데이터 흐름의 전반을 튜닝해보는 값진 기회였습니다.  

특히 **Streamlit**이라는 편리하지만 렌더링 라이프사이클이 독특한 프레임워크를 조작하며 쿠키 제어, iframe 샌드박스 보안 충돌, 무한 루프, 스레드 격리 문제를 현업 수준에서 집요하게 추적하여 해결할 수 있었습니다.

또한 최근 AI 애플리케이션 개발에 필수적인 **Agent의 답변 품질 검증을 Ragas 평가 모델을 적용하여 자동화**해보고, 대량의 데이터 수집을 방지하기 위해 **파이프라인을 모듈별로 격리 및 스케줄러로 제어**해보는 등의 데이터 엔지니어링 패턴을 깊게 이식해 본 것이 매우 유익했습니다. 이러한 경험은 향후 엔지니어로서 실무에서 더욱 견고하고 신뢰성 높은 데이터를 취급하고 보장하는 데 훌륭한 밑거름이 될 것이라 확신합니다.
...
<details>
<summary><b>프로젝트 회고 - '돌아가는 코드'를 넘어 '안정적인 시스템'으로 (클릭하여 펼치기)</b></summary>
<br>
...
**[Mobile Project (Smart Pot)](https://github.com/EJ-pro/smartpot)** — 식물 다이어리 & IoT 하드웨어 연동 안드로이드 앱
